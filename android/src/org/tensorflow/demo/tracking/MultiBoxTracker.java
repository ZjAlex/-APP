...
import org.tensorflow.demo.RankModel;
...
public class MultiBoxTracker {
  ...
  private static final int[] COLORS = {
    ...
    Color.parseColor("#AA33AA"), Color.parseColor("#0D0068"),Color.parseColor("#2D0368"),
          Color.parseColor("#0D3148"),Color.parseColor("#1D0238"),Color.parseColor("#0D0068"),
          Color.parseColor("#0D3168"),Color.parseColor("#0Df063"),
          Color.parseColor("#0D0668"),Color.parseColor("#0350ff")
  };
  ...
  public synchronized void draw(final Canvas canvas, final Bitmap bit) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
     /* RectF trackedPos =
      ...
      float[] point1 = recognition.point1;
      float[] point2 = recognition.point2;
      float[] point3 = recognition.point3;
      float[] point4 = recognition.point4;
      float[] point5 = recognition.point5;

      getFrameToCanvasMatrix().mapPoints(point1);
      getFrameToCanvasMatrix().mapPoints(point2);
      getFrameToCanvasMatrix().mapPoints(point3);
      getFrameToCanvasMatrix().mapPoints(point4);
      getFrameToCanvasMatrix().mapPoints(point5);

      if (point1[0] <= trackedPos.left || point1[0] >= trackedPos.right || point1[1] <= trackedPos.top || point1[1] >= trackedPos.bottom)
      {
        point1[0] = 0.0f;
        point1[1] = 0.0f;
      }
      if (point2[0] <= trackedPos.left || point2[0] >= trackedPos.right || point2[1] <= trackedPos.top || point2[1] >= trackedPos.bottom)
      {
        point2[0] = 0.0f;
        point2[1] = 0.0f;
      }
      if (point3[0] <= trackedPos.left || point3[0] >= trackedPos.right || point3[1] <= trackedPos.top || point3[1] >= trackedPos.bottom)
      {
        point3[0] = 0.0f;
        point3[1] = 0.0f;
      }
      if (point4[0] <= trackedPos.left || point4[0] >= trackedPos.right || point4[1] <= trackedPos.top || point4[1] >= trackedPos.bottom)
      {
        point4[0] = 0.0f;
        point4[1] = 0.0f;
      }
      if (point5[0] <= trackedPos.left || point5[0] >= trackedPos.right || point5[1] <= trackedPos.top || point5[1] >= trackedPos.bottom)
      {
        point5[0] = 0.0f;
        point5[1] = 0.0f;
      }
      canvas.drawCircle(point1[0], point1[1], 2, boxPaint);
      canvas.drawCircle(point2[0], point2[1], 2, boxPaint);
      canvas.drawCircle(point3[0], point3[1], 2, boxPaint);
      canvas.drawCircle(point4[0], point4[1], 2, boxPaint);
      canvas.drawCircle(point5[0], point5[1], 2, boxPaint);
      ...
      final String labelString =
              !TextUtils.isEmpty(recognition.title)
                      ? String.format("%s %.2f", "face", recognition.detectionConfidence)
                      : String.format("%.2f", recognition.detectionConfidence);
  ...
  private void processResults(
    ...
    //if (objectTracker == null)
    {
      ...
      for (final Pair<Float, Recognition> potential : rectsToTrack) {
        ...
        trackedRecognition.rank = potential.second.getRank();
        trackedRecognition.point1 = potential.second.getPoint1();
        trackedRecognition.point2 = potential.second.getPoint2();
        trackedRecognition.point3 = potential.second.getPoint3();
        trackedRecognition.point4 = potential.second.getPoint4();
        trackedRecognition.point5 = potential.second.getPoint5();
